...
randpar(μ::Float64, min::Float64, max::Float64, cv::Float64 = 5.0) = exp10(rand(Distributions.TruncatedNormal(log10(μ), (log10(max) - log10(min)) / cv, log10(min), log10(max))))

function rand_clon(parent::Tumor, id::Int)
	g = randpar(parent.g, 0.01, 42)
	λ = randpar(parent.d, 1e-5, 1000)
	d = randpar(parent.d, 0.01, 42)
...
function renovate!(tumor::Set{Tumor}, last_id::Int)
	...
	for clon in tumor
		...
		for k = 1 : rand(Distributions.Poisson(mutprob))
			last_id += 1
			push!(childs, rand_clon(clon, last_id))
		end
	end
...
function main()
	...
	initdata = readdlm(args["init"])
	n_clonesT_0 = convert(Int, initdata[1])
	@assert length(initdata) == (Npeptidos + 2) * n_clonesT_0 + 1
	...
	for i = 1 : n_clonesT_0
		e = initdata[i + 1]
		r = initdata[n_clonesT_0 + i + 1]
		k = initdata[1 + 2n_clonesT_0 + Npeptidos * (i - 1) + (1 : Npeptidos)]
		Aff = 0.0
		push!(SI, TCell(e, r, k, 0.0))
	end
